...
public class HttpsUtil extends DefaultHttpClient {
	...
	public void initSSLConfigForTwoWay() throws Exception {
		...
		String selfcertpath = HttpConstant.SELFCERTPATH;
		String trustcapath = HttpConstant.TRUSTCAPATH;

		String path = ResourceUtils.getURL("classpath:").getPath();

		KeyStore selfCert = KeyStore.getInstance("pkcs12");

		File selfcertpathFile = new File(path + selfcertpath);
		File trustcapathFile = new File(path + trustcapath);
		InputStream selfcertpathInput = null;
		InputStream trustcapathFileInput = null;
		try {
			selfcertpathInput = new FileInputStream(selfcertpathFile);

			/*之前的获取安全证书的方法，这儿获取不到
			selfCert.load(Thread.currentThread().getContextClassLoader().getResourceAsStream(path + selfcertpath),Constant.SELFCERTPWD.toCharArray());*/
			selfCert.load(selfcertpathInput, HttpConstant.SELFCERTPWD.toCharArray());
			KeyManagerFactory kmf = KeyManagerFactory.getInstance("sunx509");
			kmf.init(selfCert, HttpConstant.SELFCERTPWD.toCharArray());

			// 2 Import the CA certificate of the server,
			KeyStore caCert = KeyStore.getInstance("jks");
			trustcapathFileInput = new FileInputStream(trustcapathFile);
			/*caCert.load(Thread.currentThread().getContextClassLoader().getResourceAsStream(path+trustcapath), Constant.TRUSTCAPWD.toCharArray());*/
			caCert.load(trustcapathFileInput, HttpConstant.TRUSTCAPWD.toCharArray());
			...
			selfcertpathInput.close();
			trustcapathFileInput.close();
		}
